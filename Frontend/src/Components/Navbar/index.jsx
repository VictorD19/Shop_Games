...
import { loginUser, registerUser } from "../../Api/userEndpoint";
...
export const Navbar = () => {
  ...
  const handleLogout = () => dispatch({ method: "LOGOUT" });
  const handleLogin = async (dataForm) => {
    ...
    try {
      const { user, token, error } = await registerUser(dataForm);
      if (error) throw new Error(error);
      createCookie("token", token);
      dispatch({ method: "INITIAL_USER", data: user });
      handleCloseModal();
  ...
  return (
    <>
          ...
          {user?.email && (
            <>
              ...
              >
                ...
                <Dropdown.Item onClick={() => navigate("/my_games")}>
                  My games
                </Dropdown.Item>
      ...
      <RightMenu show={show} onHide={handleClose}>
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>SHOP GAMES</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          <ListGroup variant="flush">
            ...
            {token && (
              <ListGroup.Item onClick={handleLogout}>Sair</ListGroup.Item>
            )}
          </ListGroup>
        </Offcanvas.Body>
      </RightMenu>
